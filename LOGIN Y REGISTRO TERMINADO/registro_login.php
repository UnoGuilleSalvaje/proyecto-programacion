...
function verificar_login($conexion, $username, $password, $remember_me){
    ...
    $stmt = $conexion->prepare("SELECT id, hashed_password, intentos_fallidos, artista FROM usuarios WHERE username = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $stmt->bind_result($user_id, $stored_hashed_password, $intentos_fallidos, $artista);
    $stmt->fetch();
    $stmt->close();
    ...
    if ($remember_me) {
        // Establecer cookies (aquí debes implementar la lógica para establecer las cookies)
    ...
    ///Aqui mandarlo al menu
        ...
        echo "Inicio de sesión fallido. Nombre de usuario o contraseña incorrectos.<br>";
          echo "<p><a href='index.php'>Volver</a></p>";
          echo "</div>";
        // Verificar si la cuenta está bloqueada y mostrar la pregunta de seguridad
        if ($intentos_fallidos >= $max_intentos) {
            mostrar_pregunta_seguridad($artista,$username);
        }
    }
